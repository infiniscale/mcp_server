...
func NewFilesystemServer(allowedDirs []string) (*server.MCPServer, error) {
	...
	s.AddTool(mcp.NewTool(
		"croc_send",
		mcp.WithDescription("Send a file or folder to another machine using croc. Returns a code that the recipient needs to receive the file. The croc process runs in the background waiting for a recipient."),
		mcp.WithString("path",
